...
const Scene = () => {
  ...
  // Texture loading with proper parameters
  ...
  const loadTexture = (path) => {
    ...
    tex.flipY = false; // Critical for Shadertoy compatibility
    return tex;
  };

  const channel0 = loadTexture('/texture11.png');
  const channel1 = loadTexture('/texture00.jpg');

  useEffect(() => {
    setLoaded(true);
  }, []);

  // Shader material with error checking
...
const Render2 = () => {
  return (
    <div style={{ width: '780px', height: '300px',position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -72%)",zIndex: 2,pointerEvents: 'none' }}>
      <Canvas style={{pointerEvents: 'none'}} camera={{ position: [0, 0, 1], near: 0.1, far: 10 }}>
        <Scene />
      </Canvas>
    </div>
  );
